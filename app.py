...
if 'user' not in st.session_state:
    with st.sidebar:
        ...
        choice = st.sidebar.radio("Navegación", ["Inicio", "Acerca de", "Contacto", "Registrarse", "Iniciar Sesión"])
        st.divider()
    ...
    # --- SECCIÓN: ACERCA DE (REINTEGRADO) ---
        ...
        with col_text:
            ...
            * **Hecho por y para maestros:** Entendemos los Campos Formativos, los Ejes Articuladores y el diseño de los Proyectos.
            * **Tecnología de Vanguardia:** Utilizamos el motor de IA más rápido del mundo para que tus planeaciones estén listas en segundos.
            * **Compromiso Social:** Creemos en la educación pública y en dotar a sus maestros de las mejores herramientas digitales.
            """)
        with col_img:
            st.image("https://images.unsplash.com/photo-1544531585-9847b68c8c86?auto=format&fit=crop&w=500", 
                     caption="Transformando la educación con tecnología", use_container_width=True)
        st.info("💡 **Dato curioso:** Con Maestro Estratega NEM, un docente promedio recupera hasta 8 horas a la semana de su tiempo personal.")
    ...
    # --- SECCIÓN: REGISTRO ---
        ...
        with st.form("reg"):
            ...
            confirm_reg = st.text_input("Confirmar contraseña", type="password")
            if st.form_submit_button("Crear Cuenta Gratuita"):
                if pw_reg == confirm_reg and len(pw_reg) > 5:
                    registrar_usuario(email_reg, pw_reg)
                    st.success("Cuenta creada exitosamente. Ahora ve a 'Iniciar Sesión'.")
                else: st.error("Error: Revisa que las contraseñas coincidan (mínimo 6 caracteres).")
